...
void geometry_hit_set_face_normal(Hit *hit, const Ray ray, const Vec3 outward_normal) {
    // Sets the hit record normal vector.
    // NOTE: the parameter `outward_normal` is assumed to have unit length.

    hit->front_face = vec3_dot(ray.direction, outward_normal) < 0;
    hit->normal = hit->front_face ? outward_normal : vec3_scale(outward_normal, -1);
}
...
bool geometry_sphere_intersect(const Ray ray, const Sphere sphere, Interval ray_t, Hit *hit) {
    ...
    Vec3 outward_normal = vec3_divide(vec3_subtract(hit->point, sphere.center), sphere.radius);
