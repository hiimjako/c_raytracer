...
void world_add(World* list, Sphere object) {
    ...
    if (list->size == list->capacity) {
        list->capacity *= 2;
        Sphere* new_data = (Sphere*)realloc(list->data, sizeof(Sphere) * list->capacity);
        if (new_data == NULL) {
...
bool world_hit(const World* list, const Ray ray, Interval ray_t, Hit* hit) {
    ...
    for (size_t i = 0; i < list->size; ++i) {
        ...
        if (geometry_sphere_intersect(ray, sphere, interval_create(ray_t.min, closest_so_far), &temp_rec)) {
            hit_anything = true;
